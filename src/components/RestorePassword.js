...
export default function RestorePassword() {
  ...
  function View() {
    ...
    this.createPage = () => {
      ...
      ui.container.append(this.getInfo());
      ui.container.append(this.getForm());
      ui.submitBtn = ui.container.querySelector(".btn");
      ui.form.ui.submitBtn.element.addEventListener("click", (event) => {
        event.preventDefault();
        if (ui.form.validate())
          this.success(
            ui.form.ui.emailInput.element.querySelector("input").value
          );
      });
      return ui.container;
    };
    ...
    this.getTitle = () => {
      ...
      link.href = "signIn";
      link.innerHTML = this.getIcon();
      Router.makeLink(link);
      title.className = "ui-title";
      title.append(link, "Restore password");
      return title;
    };

    this.getInfo = () => {
      ui.info = document.createElement("p");
      ui.info.className = "ui-info";
      ui.info.textContent =
    ...
    this.getForm = () => {
      ui.form = new Form().init({ type: "restorePassword" });
    ...
    this.success = (email) => {
      ui.form.element.remove();
      const link = document.createElement("a");
      link.className = "link";
      link.textContent = email;
      ui.info.innerHTML = `An email has been sent to ${link.outerHTML}. Check your inbox, and click the reset link provided.`;
    };
  }
