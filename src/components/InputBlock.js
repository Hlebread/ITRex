export default function InputBlock() {
  ...
  function View() {
    ...
    this.createComponent = () => {
      ui.component = document.createElement("div");
      ui.component.className = "auth-form__inp-block";
      ui.component.innerHTML = this.getContent();
      ui.input = ui.component.querySelector(".inp");
      if (config.type === "password" || config.type === "password-check") {
        ui.eye = ui.component.querySelector(".icon_show-pass");
        ui.eye.addEventListener("click", () => this.showPassword());
      }
      ui.invalidMessage = document.createElement("div");
      ui.invalidMessage.className = "auth-form__inp-block__invalid-message";
      ui.invalidMessage.textContent = config.invalidMessage || "Invalid text";
      ui.component.append(ui.invalidMessage);
      return ui.component;
    };

    this.getContent = () => {
      const icon = config.type ? this.getIcon(config.type) : "";
      const placeholder = config.placeholder || "Type a text";
      const id = config.type ? `${config.type}-input` : "";
      let type = "text";
      let eye = "";
      if (config.type === "password" || config.type === "password-check") {
        eye = this.getIcon("eye");
        type = "password";
      }

      return `<label class="auth-form__inp-block__container">
						${icon}
						<input
						id="${id}"
						type="${type}"
    ...
    this.showPassword = () => {
      const type = ui.input.getAttribute("type");
      if (type === "text") {
        ui.input.setAttribute("type", "password");
      } else if (type === "password") {
        ui.input.setAttribute("type", "text");
      }
    };

    this.setInvalid = (invalidMessage) => {
      ui.component.classList.add("auth-form__inp-block_invalid");
      if (invalidMessage) ui.invalidMessage.textContent = invalidMessage;
    };

    this.setValid = () => {
      ui.component.classList.remove("auth-form__inp-block_invalid");
    };
  }
  ...
  return {
    init: function (config) {
      ...
      return {
        ...
        setInvalid: componentController.setInvalid,
        setValid: componentController.setValid,
      };
    },
  };
}
